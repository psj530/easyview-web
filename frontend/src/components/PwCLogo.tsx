/**
 * PwC Official Logo SVG Component (2025 version)
 * Uses the global PwC logo used by 삼일회계법인 (Samil PwC Korea).
 * Includes the "pwc" logotype + momentum mark (two orange parallelograms).
 */

interface PwCLogoProps {
  height?: number;
  /** Color for the "pwc" text. Default dark (for light backgrounds). */
  color?: string;
  className?: string;
}

export default function PwCLogo({
  height = 24,
  color = "#2D2D2D",
  className = "",
}: PwCLogoProps) {
  // viewBox: x=14, y=32, w=73, h=36 — covers pwc text (x15-72) + momentum mark (x48-85)
  const scale = height / 36;
  const width = Math.round(73 * scale);

  return (
    <svg
      width={width}
      height={height}
      viewBox="14 32 73 36"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      aria-label="PwC"
    >
      {/* "pwc" logotype */}
      <path
        d="M66.7 47.3001C64.2 47.7001 63 49.5001 63 52.7001C63 55.9001 64.7 58.1001 67.2 58.1001C69.7 58.1001 69.5 57.7001 71.8 56.6001V59.2001C69.1 60.5001 67.5 60.8001 65.2 60.8001C62.9 60.8001 61.1 60.2001 59.8 58.8001C58.4 57.4001 57.7 55.5001 57.7 53.5001C57.7 48.9001 61.1 45.8001 66.1 45.8001C71.1 45.8001 71.7 47.3001 71.7 49.5001C71.7 51.7001 70.6 51.9001 69.1 51.9001C67.6 51.9001 67.6 51.7001 66.8 51.2001V47.3001H66.7ZM54.6 53.4001C56.8 50.6001 57.6 49.5001 57.6 48.1001C57.6 46.7001 56.5 45.6001 55.1 45.6001C53.7 45.6001 53.4 46.0001 53 46.5001V52.2001L49.4 57.0001V46.0001H46L40.3 55.5001V46.0001H38.3L33.1 47.3001V48.6001L35.9 48.9001V60.5001H39.6L45.1 51.5001V60.5001H49.1L54.7 53.4001H54.6ZM22.2 49.0001C23 49.0001 23.5 48.8001 23.9 48.8001C26.3 48.8001 27.6 50.4001 27.6 53.4001C27.6 56.4001 26 58.8001 23.1 58.8001C20.2 58.8001 22.7 58.8001 22.3 58.8001V49.1001L22.2 49.0001ZM22.2 60.4001C23.1 60.4001 24.1 60.4001 24.6 60.4001C29.5 60.4001 32.6 57.3001 32.6 52.6001C32.6 47.9001 30.3 45.7001 27.1 45.7001C23.9 45.7001 24.8 46.0001 22.2 47.6001V45.7001H20.7L15 47.4001V48.8001H17.4V65.0001L15.3 65.5001V66.8001H24.6V65.5001L22.2 65.0001V60.3001V60.4001Z"
        fill={color}
      />
      {/* Momentum mark (two orange parallelograms) */}
      <path
        d="M64.1 41.8H48.6L51.2 37.4H66.7L64.1 41.8ZM84.9 33H69.4L66.8 37.4H82.3L84.9 33Z"
        fill="#D04A02"
      />
    </svg>
  );
}
